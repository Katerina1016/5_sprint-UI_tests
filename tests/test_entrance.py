...
from locators import MainPageLocators, AuthorizationPageLocators, RegistrationPageLocators, PasswordRecoverPageLocators
...
class TestLogin:
    def test_login_by_login_button_success(self, driver):  # Вход в личный кабинет через кнопку "Войти"
        ...
        WebDriverWait(driver, 10).until(ec.visibility_of_element_located(MainPageLocators.place_an_order_but))
    ...
    def test_login_by_recover_form_success(self, driver):  # Вход в личный кабинет
        driver.get(URLS.PASSWORD_RECOVERY_PAGE)
        driver.find_element(*PasswordRecoverPageLocators.login_account_but).click()
        driver.find_element(*AuthorizationPageLocators.email_input).send_keys(User.email)
        driver.find_element(*AuthorizationPageLocators.password_input).send_keys(User.password)
        driver.find_element(*AuthorizationPageLocators.login_account_auth_form_but).click()
